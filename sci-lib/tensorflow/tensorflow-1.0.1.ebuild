...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="gcp go hdfs jemalloc lib +python cpu_flags_x86_sse cpu_flags_x86_sse2 cpu_flags_x86_sse3 cpu_flags_x86_sse4_1 cpu_flags_x86_sse4_2 cpu_flags_x86_avx cpu_flags_x86_avx2 cpu_flags_x86_avx512f cpu_flags_x86_fma3 cpu_flags_x86_fma4"
...
src_compile() {
    ...
    if use python; then
	    do_compile() {
		    cd "${BUILD_DIR}" || die
		    bazel --output_base="${bazel_base}" --batch build --nofetch --config opt //tensorflow/tools/pip_package:build_pip_package || die
	    }
    ...
    if use lib; then
        do_libcompile() {
		    cd "${BUILD_DIR}" || die
	        bazel --output_base="${bazel_base}" --batch build --nofetch --config opt //tensorflow:libtensorflow.so || die
        }
